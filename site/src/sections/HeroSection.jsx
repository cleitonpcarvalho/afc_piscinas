...
const BADGES = [
  { icon: ShieldCheck, label: 'Licenciados e Segurados' },
  { icon: Award,       label: 'Garantia de Qualidade' },
  { icon: Wrench,      label: 'Inspecção Gratuita' },
]
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-3xl mx-auto px-4 sm:px-6 text-center pt-24 pb-20">
        <span className="section-eyebrow">Construção · Renovação · Manutenção</span>

        <h1 className="text-5xl md:text-6xl lg:text-7xl font-extrabold leading-tight mb-6 text-white">
          Projetamos e<br />
          <span className="text-accent">Construímos Piscinas</span>
        </h1>

        <p className="text-white/80 text-lg md:text-xl leading-relaxed mb-10 max-w-xl mx-auto">
          AFC Piscinas — empresa reconhecida e especializada em construção, renovação e manutenção
          de piscinas, saunas, spas e banhos turcos em Portugal.
        </p>
      ...
      >
        ...
        <div>
          ...
          <p className="text-white font-bold text-sm">+{s.whatsapp_number?.replace(/(\d{3})(\d{2})(\d{3})(\d{3})(\d{2})/, '$1 $2 $3 $4 $5') || '351 96 733 57 07'}</p>
        </div>
      </a>
    </section>
  )
